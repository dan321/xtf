...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:import href="../crossQuery/queryParser/default/queryParser.xsl"/>
   ...
   <!-- 
   Template copied from style/dynaXML/docReqParser.xsl in order to change the relative path to the data files
   which is computed relative to this stylesheet which is one level down in a directory tree from the main XTF stylesheets,
   i.e. local/style/dynaXML instead of style/dynaXML
   -->
   <xsl:template match="/">
      ...
      -->
      <style path="{
         if (matches($http.URL, $ercPat)) then 'local/style/dynaXML/docFormatter/erc/ercDocFormatter.xsl'
         else if ($fileType = 'ead') then 'local/style/dynaXML/docFormatter/ead/eadDocFormatter.xsl'
         else if ($fileType = 'nlm') then 'local/style/dynaXML/docFormatter/nlm/nlmDocFormatter.xsl'
         else if ($fileType = 'tei') then 'local/style/dynaXML/docFormatter/tei/teiDocFormatter.xsl'
         else if ($fileType = 'book') then 'local/style/dynaXML/docFormatter/bookreader/bookDocFormatter.xsl'
         else                             'local/style/dynaXML/docFormatter/default/docFormatter.xsl'}"/>
      ...
      -->
      <preFilter path="{
         if      ($fileType = 'ead') then 'local/style/textIndexer/ead/eadPreFilter.xsl'
         else if ($fileType = 'nlm') then 'local/style/textIndexer/nlm/nlmPreFilter.xsl'
         else if ($fileType = 'tei') then 'local/style/textIndexer/tei/teiPreFilter.xsl'
         else                             'local/style/textIndexer/default/defaultPreFilter.xsl'}"/>
